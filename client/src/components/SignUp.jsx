...
function Frame3() {
  return <div className="absolute bg-white h-[59px] left-[78px] right-[86px] rounded-[40px] top-[37px]">
    <input placeholder='Username' className='bg-[#1c1c29] placeholder-white w-full h-full px-6 rounded-[40px] outline-none border-none' type="text" id="user" name='username'/>
  </div>;
}

function Frame4() {
  return <div className="absolute bg-white h-[59px] left-[78px] right-[86px] rounded-[40px] top-[133px]">
    <input placeholder='Email' className='bg-[#1c1c29] placeholder-white w-full h-full px-6 rounded-[40px] outline-none border-none' name="email" type="text" id="email"/>
  </div>;
}

function Frame5() {
  return <div className="absolute bottom-[89px] h-[59px] left-1/2 rounded-[20px] translate-x-[-50%] w-[319px]">
    <input className='bg-[#4338CA] placeholder-white w-full h-full px-6 rounded-[40px] outline-none border-none text-white' id='Sign' type="submit" value="Begin"  name='button'/>
  </div>;
}

function Frame6() {
  return <div className="absolute bg-white h-[59px] left-[78px] rounded-[40px] translate-y-[-50%] w-[234px]" style={{ top: "calc(50% + 0.5px)" }}>
    <input placeholder='Password' className='bg-[#1c1c29] placeholder-white w-full h-full px-6 rounded-[40px] outline-none border-none' name="password" type='password' id="pass"/>
  </div>;
}

function Frame7() {
  return <div className="absolute bg-white h-[59px] right-[86px] rounded-[40px] translate-y-[-50%] w-[234px]" style={{ top: "calc(50% + 0.5px)" }}>
    <input placeholder='Confirm Password' className='bg-[#1c1c29] placeholder-white w-full h-full px-6 rounded-[40px] outline-none border-none' name="repeatPassword" type="password" id="cpass"/>
  </div>;
}
...
const SignUp = ({user, CreateCookie}) => {
  ...
  return (
    <form onSubmit={submit} id='SignUp' className="bg-[#292838] content-stretch flex flex-col items-start relative size-full h-screen w-screen overflow-hidden">
        ...
        <div className="absolute inset-[-2.08%_-1.67%_-2.94%_-1.69%]">
          <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 1436 982">
            <g filter="url(#filter0_d_11_20)" id="Vector 1">
              <path d={svgPaths.p1c7c8700} fill="var(--fill-0, #1E1A4D)" />
      ...
      <div className="bg-[#15141b] absolute backdrop-blur-[6.7px] backdrop-filter h-[518px] overflow-clip rounded-[40px] shadow-[0px_4px_17.1px_1px_rgba(0,0,0,0.25)] translate-x-[-50%] translate-y-[-50%] w-[695px]" style={{ top: "calc(50% + 78.5px)", left: "calc(50% - 1.5px)" }}>
      <Frame3 />
      <Frame4 />
      <Frame5 />
      ...
      <div className="absolute bottom-[41px] flex flex-col font-['Inter:Bold',_sans-serif] font-bold justify-center leading-[0] left-1/2 not-italic text-[#2b2b2b] text-[20px] text-center text-nowrap tracking-[-0.5px] translate-x-[-50%] translate-y-[50%]">
        <Link to={`/login`} style={{textDecoration: 'none'}}>
          <p className="text-[#ffffff] leading-[1.1] whitespace-pre">Log in</p>
        </Link>
      </div>
      <Frame6 />
      <Frame7 />
      ...
      <div className="[text-shadow:rgba(0,0,0,0.25)_0px_4px_4px] absolute flex flex-col font-['Inter:Bold',_sans-serif] font-bold h-[62px] justify-center leading-[0] left-1/2 not-italic text-[#2b2b2b] text-[56px] text-center top-[164px] tracking-[-1.4px] translate-x-[-50%] translate-y-[-50%] w-[600px]">
        <div className=" text-white leading-[1.1]">Begin Your Journey</div>
      </div>
    </form>
  )
}
