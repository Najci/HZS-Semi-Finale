...
const FoodSmall = ({Img, Name, ClickAction, Count}) => {
  return (
    <div style={{height: `calc(var(--spacing) * 10)`, width: `calc(var(--spacing) * 66.9)`}} className='bg-[#1c1c29] hover:bg-[#2c2c41] transition flex flex-row justify-between p-2 pl-3 pr-3 rounded-3xl' onClick={() => ClickAction()}>
...
const FoodInventoryItem = ({Img, Name, ClickAction, Count}) => {
  return (
    <div className='bg-[#1c1c29] w-6/13 flex flex-row justify-between p-2 pl-3 pr-3 rounded-3xl' onClick={() => ClickAction()}>
      <p className='text-white w-1/3'>{Name}</p>
